...
class Conventional(PGMconfiguration):
    def _define_comps(self):
        self.comps["fuse"] = PGMbody(
            num_x=12, num_y=4, num_z=2
        )  # num_x is the number of surfacees
        ...
        self.comps["lwing_fuse"] = PGMjunction(
            self, "fuse", "lft", "E", [2, 1], "lwing", "right"
        )
        self.comps["rwing_fuse"] = PGMjunction(
            self, "fuse", "rgt", "W", [2, 5], "rwing", "left"
        )
        self.comps["lpylon_lwing"] = PGMjunction(
            self, "lwing", "low", "N", [1, 0], "lpylon", "right"
        )
        self.comps["lpylon_lnac"] = PGMjunction(
            self, "lnac", "tp0", "W", [1, 0], "lpylon", "left"
        )
        self.comps["ltail_fuse"] = PGMjunction(
            self, "fuse", "lft", "E", [1, 9], "ltail", "right"
        )
        self.comps["rtail_fuse"] = PGMjunction(
            self, "fuse", "rgt", "W", [1, 0], "rtail", "left"
        )
        self.comps["vtail_fuse"] = PGMjunction(
            self, "fuse", "top", "E", [0, 8], "vtail", "right"
        )
    ...
    def _define_params(self):
        ...
        fuse["pos"].params[""] = PGMparameter(
            2, 3
        )  # Specify the cylinder front and end
        ...
        fuse["pos"].params["nose"] = PGMparameter(
            3, 3, pos_u=[0, 0.065, 0.13], order_u=3
        )
        fuse["scl"].params["nose"] = PGMparameter(
            3, 1, pos_u=[0, 0.07, 0.14], order_u=3
        )
    ...
    def _define_dvs(self):
        ...
        dvs["mid_chord"] = PGMdv((1), 4.5).set_identity_param(
            "lwing", "scl", "", (1, 0)
        )
        dvs["tip_chord"] = PGMdv((1), 1.2).set_identity_param(
            "lwing", "scl", "", (2, 0)
        )
        dvs["shape_wing_upp"] = PGMdv((10, 6)).set_identity_param(
            "lwing", ("shY", "upp"), ""
        )
        dvs["shape_wing_low"] = PGMdv((10, 6)).set_identity_param(
            "lwing", ("shY", "low"), ""
        )
        dvs["lwing_fuse_normal"] = PGMdv((7, 7)).set_identity_param(
            "lwing_fuse", ("shN", ""), ""
        )
    ...
    def _compute_params(self):
        ...
        lwing_fuse = self.comps["lwing_fuse"].props
        lwing_fuse["shN", ""].params[""].data[
            :, :
        ] = 0.0  # No normal perturbation initially
    ...
    def _set_bspline_options(self):
        ...
        comps["fuse"].faces["rgt"].set_option(
            "num_cp", "v", [18, 4, 4, 4, 4, 8, 4, 15, 4, 4, 10, 4]
        )
        comps["fuse"].faces["rgt"].set_option(
            "num_pt", "v", [40, 16, 16, 16, 16, 60, 16, 60, 16, 16, 70, 16], both=False
        )
    ...
    def meshStructure(self):
        ...
        for j in range(idims.shape[0] - 1):
            ...
            afm.addVertFlip(
                "Mlw_sec2:" + str(j), "lwing", [idims[j], jdims[j]], [0.45, jdims[j]]
            )
            afm.addVertFlip(
                "Mrw_sec2:" + str(j),
                "rwing",
        ...
        for i in range(idims.shape[0]):
            if i == 0 or i == idims.shape[0] - 1:
                afm.addCtrVert(
                    "Mcw_2:" + str(i) + ":" + str(j), "lwing", "rwing", idims[i]
                )
            else:
                afm.addCtrVert(
                    "Mcw_2a:" + str(i) + ":" + str(j),
...
if __name__ == "__main__":
    ...
    pgm.dvs["lwing_fuse_normal"].data[
        :, :
    ] = 0.0  # Select normal perturbation for wing-fuselage junction
    pgm.compute_all()
    pgm.compute_normals()
    pgm.compute_all()
